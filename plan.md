...
2.  **Data Ingestion:**
    ...
    *   Initialize empty sets to collect unique subjects and qualifications: `unique_subjects = set()`, `unique_qualifications = set()`.
3.  **Processing Loop:**
    ...
    *   **Process Eligibility Rules:** For each `rule` in the course's `eligibility_rules` array:
        ...
        *   Add all subjects from the `required_subjects` list to the `unique_subjects` set.
...
*   **Logic:**
    ...
    *   Implements the logic for `stream`:
...
3.  **The Router:** Use an `if/elif/else` block to check the `intent` from the `llm_output`.
    ...
    *   **If `intent == 'check_eligibility'`:**
        ...
        5.  Clean the `percentage` value.
        6.  Call `search_engine.find_by_eligibility(normalized_criteria)`.
